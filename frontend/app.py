...
def show_ai_chatbot_page():
    ...
    with st.sidebar:
        ...
        news_source = st.selectbox(
            "News Source:",
            ["Auto", "Yahoo Finance", "NewsAPI", "RSS Feeds"],
...
- **NewsAPI**: Latest news from NewsAPI's comprehensive database
...
def show_newsletter_preview(newsletter_data):
    ...
    # Display the main content in a larger, scrollable area
    content = newsletter_data.get("content", "No content available")
    
    # Create a container with custom styling for better content display
    with st.container():
        ...
        with col1:
            st.text_area(
                ...
                help="This is the complete newsletter content generated by our AI agents. Scroll to see the full content.",
                key="newsletter_content_display"
            )
        with col2:
            if st.button("📋 Copy Content", use_container_width=True, key="copy_content"):
        ...
        # Show a preview of the content in a more readable format
        if content and len(content) > 200:
            st.markdown("**📖 Content Preview (first 200 characters):**")
            st.info(content[:200] + "..." if len(content) > 200 else content)
